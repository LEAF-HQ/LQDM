...
def PlotLimits(self, xaxistitle, yaxistitle, selectiontag=None, signal_scaled_by=1., draw_observed=False, draw_theory=False):
    ...
    c = tdrCanvas(canvName='c', x_min=min(limits_and_quantiles_per_mass), x_max=max(limits_and_quantiles_per_mass), y_min=1E-2, y_max=1E4, nameXaxis=xaxistitle, nameYaxis=yaxistitle, square=True, iPeriod=0, iPos=11)
    c.SetLogy()
    ...
    c.RedrawAxis()
    outname = os.path.join(self.plotoutput_path, 'Limits%s.pdf' % ('' if selectiontag is None else selectiontag))
    c.SaveAs(outname)
    if draw_theory:
        xsecinfile.Close()
