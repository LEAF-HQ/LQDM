...
void PostAnalysisTool::ProduceCombineHistograms(){
  ...
  vector<TString> region_tags = {"cat1", "cat2"};
  vector<TString> histinname_base = {"stmet2", "stmet2"};
  vector<TString> histoutname_base = {"ST", "ST"};
  ...
  string command = "mkdir -p " + (string)m_datacard_path + "combineinput";
  system(command.c_str());

  TString outfilename = PostAnalysisTool::m_datacard_path + "combineinput/combine_histograms.root";
