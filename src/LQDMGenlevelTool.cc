...
#include "LEAF/LQDM/include/GenHists.h"

using namespace std;

class LQDMGenlevelTool : public BaseTool {

public:
  // Constructors, destructor
  LQDMGenlevelTool(const Config & cfg);
  ~LQDMGenlevelTool() = default;
  void ProcessDataset(const Config & cfg) override {LoopEvents<LQDMGenlevelTool, GenEvent>(cfg, event, *this);};
...
LQDMGenlevelTool::LQDMGenlevelTool(const Config & cfg) : BaseTool(cfg){
...
void LQDMGenlevelTool::book_histograms(vector<TString> tags){
...
void LQDMGenlevelTool::fill_histograms(TString tag){
  TString mytag = tag+"_GenLevel";
...
REGISTER_TOOL(LQDMGenlevelTool)
